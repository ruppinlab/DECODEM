...
# -*- coding: utf-8 -*-
"""
Created on Fri Sep 27 14:15:31 2024
...
use_samples = "chemo"                                                          # chemotherapy alone
# use_samples = "chemo_immuno"                                                   # chemotherapy + ICB
...
(exp_all_sc_test, resp_PR_test, cell_ids_test, 
...
print(f"""
dataset summary:
cohort      = TransNEO (n = {resp_pCR_train.size:,}) [training], Zhang et al. 2021 (n = {resp_PR_test['Patient.id'].nunique():,}) [validation-SC]
treatment   = {use_samples + 'therapy'}, response = RECIST ('CR'/'PR' vs. 'SD'/'PD')
cell types  = {', '.join(cell_types)}
#cells [SC] = {len(resp_PR_test):,}
""")
...
y_all_test  = resp_PR_test.groupby(
...
cohort = Zhang et al. 2021 (n = {y_test.size:,}) [SC]
