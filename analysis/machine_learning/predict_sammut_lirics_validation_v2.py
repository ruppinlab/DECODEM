...
for cclr_ in cclr_all_train:
    ...
    if use_samples == "all":                                                   # flag for anti-HER2 therapy
        X_train_cclr_["aHER2"] = clin_info_train["aHER2.cycles"].notna(
            ).astype(int)
        X_test_cclr_["aHER2"]  = clin_info_test["anti.her2.cycles"].notna(
            ).astype(int)
...
print(f"\ntop 20 most predictive CCIs for ARTemis + PBCP cohort = \n{featimp_final.iloc[:20, -2:]}")
...
#               **fontdict["title"]);
ax2.set_title("ARTemis + PBCP", **fontdict["title"]);
...
if svdat:
    ...
    fig_file = f"sammut_validation_lirics_{use_samples}_feature_importance_{plt_type}plot_CCI_{use_mdl}_{num_feat_max}features_{datestamp}.pdf"
...
bulk_file = "tn_valid_predictions_chemo_th0.99_ENS2_25features_3foldCVtune_23Mar2023.pkl"
...
y_preds = {"CCI": y_pred_plt_val, "Bulk": y_pred_plt_bulk}


## make performance plots.
fig_title1 = "Performance of cell-cell interaction based model"
if use_samples == "all":
    fig_title1 += " for the whole Artemis + PBCP cohorts "
elif use_samples == "chemo_targeted":
    fig_title1 += " for HER2+ patients\ntreated with chemotherapy and Trastuzumab in PBCP cohort "
elif use_samples == "chemo":
    fig_title1 += " for HER2- patients\ntreated with chemotherapy alone in Artemis + PBCP cohorts "
...
if svdat:
    ...
    fig_file = f"sammut_validation_lirics_{use_samples}_AUC_AP_bulk_CCI_{use_mdl}_{num_feat_max}features_{datestamp}.pdf"
    fig_file = fig_file.replace("CCI_", 
                     f"filteredCCI_th{conf_th}_" if filter_cci else "allCCI_")
