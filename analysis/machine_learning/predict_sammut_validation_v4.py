...
# -*- coding: utf-8 -*-
"""
Created on Wed Aug 27 15:01:27 2025
...
print(f"""
dataset summary:
cohort     = TransNEO (n = {resp_pCR_train.size:,}) [training], ARTemis + PBCP (n = {resp_pCR_test.size:,}) [validation]
...
if lddat:
    ...
    with open(res_path + res_file, mode = "rb") as file:
...
clin_info_test_sammut = pd.read_excel(sammut_path[1] + sammut_file[2], 
                                      sheet_name = "validation", header = 0, 
                                      index_col = 0)
clin_info_test_sammut_supp = pd.read_excel(sammut_path[0] + sammut_file[1], 
...
y_test_sammut = clin_info_test_sammut.loc[
    y_score_val_sammut.index, ["pCR.RD", "anti.her2.cycles"]]
y_test_sammut["pCR.RD"] = (y_test_sammut["pCR.RD"] == "pCR").astype(int)
y_test_sammut["Cohort"] = [
    "ARTEMIS" if ("A" in idx) else "PBCP" for idx in y_test_sammut.index]
