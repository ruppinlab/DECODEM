...
# -*- coding: utf-8 -*-
"""
Created on Wed Aug 27 12:22:35 2025
...
for ctp_ in cell_types + ["Bulk"]:
    X_ctp_ = exp_all[ctp_][y_all.index].copy()    
    try:                                                                       # individual cell type
        gn_ctp_ = get_conf_genes(conf_score[ctp_], th = conf_th)
        if w_by_cf:
            X_ctp_ = get_weighted_exp(X_ctp_, frac = cell_frac[ctp_])
    except:                                                                    # Bulk
        gn_ctp_ = conf_score.index.tolist()
    
    X_ctp_ = X_ctp_.loc[gn_ctp_].T
...
model  = {use_mdl}, CV = Stratified {num_splits}-fold, repititons = {num_split_rep}
