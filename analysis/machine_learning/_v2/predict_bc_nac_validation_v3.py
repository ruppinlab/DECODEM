...
with open(data_path[1] + data_file[1], "rb") as file:
    ...
    resp_E_test     = data_test_obj["resp"]
...
resp_E_test.sort_values(by = ["Patient.id", "Sample.id"], ascending = True, 
                        inplace = True)
...
validation cohort (SC) = Bassez et al. 2021 (n = {resp_E_test['Patient.id'].nunique():,}, nCells = {len(resp_E_test):,})
treatment = {use_samples + 'therapy'}, response = clonotype expansion ('E' vs. 'NE')
...
y_all_test  = resp_E_test.groupby("Sample.id").first().loc[
...
# use_ctp = "B-cells"
# use_ctp = "Bulk"
# top_cell_types = ["Endothelial", "Myeloid", "B-cells"]
...
cohort = Bassez et al. SC Cohort (n = {y_test.size})
