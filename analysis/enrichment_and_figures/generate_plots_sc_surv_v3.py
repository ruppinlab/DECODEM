...
## all plots.
fig5, ax5 = plt.subplots(figsize = (16, 12), dpi = 600, nrows = 3, 
                         ncols = 2, height_ratios = [1, 1, 1], 
                         width_ratios = [1, 1])
ax5 = dict(zip(list("ABCDEF"), ax5.ravel()))

fig_llocs5 = [[0.08, 0.44], [0.96, 0.70, 0.42]]
...
ax5["A"].get_legend().set_bbox_to_anchor([-0.50, 0.25, 0.4, 0.4]);
...
ax5["C"].get_legend().set_bbox_to_anchor([-0.50, 0.35, 0.4, 0.4]);
...
## make km plots - tcga-brca.
ax5["E"] = make_km_plot(data_grp1 = fig_data5E1, data_grp2 = fig_data5E2, 
                        stat = fig_stat5EF["ER+,HER2-"], title = fig_ttls5["E"], 
                        legend = False, ax = ax5["E"])
fig5.text(x = fig_llocs5[0][0], y = fig_llocs5[1][2], s = "E", **panel_fonts);

ax5["F"] = make_km_plot(data_grp1 = fig_data5F1, data_grp2 = fig_data5F2, 
                        stat = fig_stat5EF["TNBC"], title = fig_ttls5["F"], 
                        legend = True, ax = ax5["F"])
fig5.text(x = fig_llocs5[0][1], y = fig_llocs5[1][2], s = "F", **panel_fonts);
...
if svdat:
    ...
    fig_file5 = "all_predictions_sc_survival_chemo_immuno_th0.99_ENS2_5foldCV.pdf"
...
figS7_I, axS7_I = plt.subplot_mosaic(
    mosaic = [["A", "B"], ["C", "C"]], figsize = (18, 8), dpi = 600, 
...
# axS7_I["A"].get_legend().set_bbox_to_anchor([-0.30, 0.25, 0.4, 0.4]);
axS7_I["A"].legend([ ], [ ]);
...
if svdat:
    ...
    figS7_I.savefig(fig_path + fig_fileS7_I, dpi = "figure")
...
figS7_II, axS7_II = plt.subplots(figsize = (12, 5), dpi = 600, nrows = 1, 
                                 ncols = 2, 
...
if svdat:
    ...
    figS7_II.savefig(fig_path + fig_fileS7_II, dpi = "figure")
...
figS7_III, axS7_III = plt.subplots(figsize = (6, 5), dpi = 600, 
                                   nrows = 1, ncols = 1, 
                                   subplot_kw = {"projection": "radar"})
...
if svdat:
    ...
    figS7_III.savefig(fig_path + fig_fileS7_III, dpi = "figure")
...
fig_ttlsS8  = {"A": f"Zhang et al. (n = {fig_data5A.index.nunique()})\n", 
               "B": f"Zhang et al. (n = {fig_data5B.index.nunique()})\n", 
               "C": f"Bassez et al. (n = {fig_data5C.index.nunique()})\n", 
               "D": f"Bassez et al. (n = {fig_data5D.index.nunique()})\n", 
               "E": f"\n{surv_info3.label[0]}\n", 
...
figS8, axS8 = plt.subplots(figsize = (16, 18), dpi = 600, nrows = 7, ncols = 2)
...
if svdat:
    ...
    figS8.savefig(fig_path + fig_fileS8, dpi = "figure")
...
fig_ttlsS9  = {"A": f"Zhang et al. (n = {fig_data5A.index.nunique()})\n", 
               "B": f"Zhang et al. (n = {fig_data5B.index.nunique()})\n", 
               "C": f"Bassez et al. (n = {fig_data5C.index.nunique()})\n", 
               "D": f"Bassez et al. (n = {fig_data5D.index.nunique()})\n", 
               "E": f"\n{surv_info3.label[0]}\n", 
...
figS9, axS9 = plt.subplots(figsize = (16, 18), dpi = 600, nrows = 7, ncols = 2)
for k, (lbls, (ctp, ctp_data)) in enumerate(
...
if svdat:
    ...
    figS9.savefig(fig_path + fig_fileS9, dpi = "figure")
...
fig1_II, ax1_II = plt.subplots(figsize = (14, 7), dpi = 600, nrows = 1, 
                               ncols = 2)
...
if svdat:
    ...
    fig1_II.savefig(fig_path + fig_file1_II, dpi = "figure")
