...
## prepare data for fig 3G.
## panel G: cell type vs. top pathways.
...
fig.data3_II <- cell.types.disp %>% sapply(simplify = F, function(ctp){
    gsea.all.tn[[ctp]] %>% mutate(
        ...
        head(num.path.disp) %>% 
        dplyr::select(Pathway, NES, logP, cell.type)
}) %>% Reduce(f = rbind) %>% 
    data.frame(check.names = F) %>% 
    mutate(cell.type = cell.type %>% 
               factor(levels = cell.types.disp %>% 
                          gsub(pattern = "_", replacement = " ")))


#### -------------------------------------------------------------------------------

## make fig 3G: GSEA dotplot.
## plot parameters.
font.name <- "sans"
font.size <- c("tick" = 20, "label" = 24, "title" = 32, "plabel" = 60) / 3.0
dot.size  <- c("min" = 4, "max" = 10) / 1.5
...
## make figure.
## panel G: cell type vs. pathway dotplot.
fig.plot3_II <- ggplot(data = fig.data3_II, 
                       mapping = aes(x = cell.type, y = Pathway, size = logP)) + 
    geom_point(mapping = aes(color = NES)) + 
    geom_point(shape = 21, stroke = line.size["dot"], color = plt.clrs["base"]) + 
    xlab("") + ylab("") + ggtitle(fig.ttl3_II) + 
    scale_size_continuous(name = latex2exp::TeX("$-\\bf{log}P$", bold = T, italic = T), 
                          range = dot.size) + 
    scale_color_gradient(low = plt.clrs["min"], high = plt.clrs["max"], 
                         limits = fig.data3_II$NES %>% range %>% round, 
                         guide = guide_colorbar(frame.colour = plt.clrs["base"], 
                                                ticks.colour = plt.clrs["base"], 
