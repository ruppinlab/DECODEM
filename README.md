...
<ul> (i) **Deconvolution** where we use CODEFACS to deconvolve the bulk gene expression into nine cell-type-specific gene expression profiles encompassing malignant, immune, and stromal cell types. </ul>  
<ul> (ii) **Machine learning** where we use each cell-type-specific expression profile to build a cell-type-specific machine learning predictor of chemotherapy response. </ul>  
